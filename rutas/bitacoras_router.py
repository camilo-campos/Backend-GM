...
from modelos.modelos import GmBitacoraA
...
router = APIRouter(prefix="/bitacoras", tags=["Bitácoras Bomba A"])
...
@router.post(
    "/analisis",
    summary="Analizar bitácora con IA - Bomba A",
    description="""
Analiza el contenido de una bitácora de la Bomba A utilizando un modelo de lenguaje (LLM) de IBM WatsonX.
...
async def predecir_corriente(bitacora: BitacoraInput, db: Session = Depends(get_db)):
    try:
        ...
        bitacora_db = db.query(GmBitacoraA).filter(GmBitacoraA.id == bitacora.id_bitacora).first()
        if not bitacora_db:
...
@router.get(
    "/todas",
    summary="Obtener todas las bitácoras - Bomba A",
    description="""
Obtiene las últimas 40 bitácoras de la Bomba A con clasificación automática.
...
@router.get(
    "/todas_fallas",
    summary="Obtener bitácoras con fallas HRSG - Bomba A",
    description="""
Obtiene las bitácoras de la Bomba A que han sido clasificadas como "HRSG Pump Failures" (Fallas de Bomba HRSG).
